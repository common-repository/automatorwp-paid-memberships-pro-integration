...
class AutomatorWP_Paid_Memberships_Pro_Add_User_Membership extends AutomatorWP_Integration_Action {
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Add user to membership level', 'automatorwp-paid-memberships-pro' ),
            'select_option'     => __( 'Add user to <strong>membership level</strong>', 'automatorwp-paid-memberships-pro' ),
            /* translators: %1$s: Post title. */
            'edit_label'        => sprintf( __( 'Add user to %1$s', 'automatorwp-paid-memberships-pro' ), '{membership}' ),
            /* translators: %1$s: Post title. */
            'log_label'         => sprintf( __( 'Add user to %1$s', 'automatorwp-paid-memberships-pro' ), '{membership}' ),
            'options'           => array(
                'membership' => automatorwp_utilities_ajax_selector_option( array(
                    'field'             => 'membership',
                    'name'              => __( 'Membership Level:', 'automatorwp-paid-memberships-pro' ),
                    'option_default'    => __( 'membership level', 'automatorwp-paid-memberships-pro' ),
                    'option_none'       => false,
                    'option_custom'         => true,
                    'option_custom_label'   => __( 'Membership Level ID', 'automatorwp-paid-memberships-pro' ),
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        if ( ! empty( $current_level ) && absint( $current_level->ID ) === $membership_id ) {
            $this->result = __( 'User is already on this membership level.', 'automatorwp-paid-memberships-pro' );
            return;
        }
        ...
        if( ! $new_level ) {
            $this->result = __( 'Could not add user to membership level.', 'automatorwp-paid-memberships-pro' );
            return;
        }

        $this->result = __( 'User added to membership level successfully.', 'automatorwp-paid-memberships-pro' );
    ...
    public function log_fields( $log_fields, $log, $object ) {
        ...
        $log_fields['result'] = array(
            'name' => __( 'Result:', 'automatorwp-paid-memberships-pro' ),
            'type' => 'text',
        );
